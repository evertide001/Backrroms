...
public class ModBlockStateProvider extends BlockStateProvider {
    ...
    protected void registerStatesAndModels() {
            ...
            blockWithItem(Modblocks.BLOCK_OF_FIRE_SALT);
            blockWithItem(Modblocks.BUDDING_FIRE_SALT);
            blockWithItem(Modblocks.CONCRETE);
            blockWithItem(Modblocks.BURNSTONE);
            ...
            crossWithItem(Modblocks.CARPET_GRASS);

            directionalBlockWithItem(Modblocks.WALL_SOCKET);
            directionalBlockWithItem(Modblocks.FIRE_SALT_BUD);
    ...
    private void crossWithItem(DeferredBlock<?> deferredBlock) {
        String blockName = deferredBlock.getId().getPath();
        simpleBlockWithItem(deferredBlock.get(),
                models().cross(blockName, modLoc("block/" + blockName))
                        .renderType("cutout")
        );
    }
    ...
    private void directionalBlockWithItem(DeferredBlock<?> deferredBlock) {
        ...
        simpleBlockItem(deferredBlock.get(),
                models().getExistingFile(modLoc("blocks/" + blockName))
        );
    }
